...
class EyeDetector:
    ...
    def get_Gaze_Score(self, frame, landmarks):
        ...
        # Add this method to the EyeDetector class
        def get_ROI(self, left_corner_keypoint_num: int):
            if self.keypoints is not None:
                landmarks_list = np.array(self.keypoints)
            else:
                return None
            ...
            eye_array = np.array([
                (landmarks_list[kp_num][0, 0], landmarks_list[kp_num][0, 1]),
                (landmarks_list[kp_num + 1][0, 0], landmarks_list[kp_num + 1][0, 1]),
                (landmarks_list[kp_num + 2][0, 0], landmarks_list[kp_num + 2][0, 1]),
                (landmarks_list[kp_num + 3][0, 0], landmarks_list[kp_num + 3][0, 1]),
                (landmarks_list[kp_num + 4][0, 0], landmarks_list[kp_num + 4][0, 1]),
                (landmarks_list[kp_num + 5][0, 0], landmarks_list[kp_num + 5][0, 1])
            ], np.int32)
