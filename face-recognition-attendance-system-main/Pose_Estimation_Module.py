...
class HeadPoseEstimator:
    ...
    def get_pose(self, frame, landmarks):
        ...
        """
        if isinstance(landmarks, np.matrix):
            landmarks = landmarks.tolist()

        if isinstance(landmarks, list) and len(landmarks) > 0 and isinstance(landmarks[0], list):
            landmarks_dlib = dlib.full_object_detection(
                dlib.rectangle(), [(int(p[0]), int(p[1])) for p in landmarks[0]])

            self.keypoints = landmarks_dlib  # dlib 68 landmarks
            self.frame = frame  # opencv image array

            # ... (the rest of your code remains unchanged)

        else:
            print("Error: Invalid landmarks format.")
            return None, None, None, None

    # Convert NumPy matrix to dlib full_object_detection type
        landmarks_dlib = dlib.full_object_detection(
        dlib.rectangle(), [(int(p[0, 0]), int(p[0, 1])) for p in landmarks.tolist()[0]])
